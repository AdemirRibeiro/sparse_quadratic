function prog_maxmin3(n,s,m,η,c,V,λ,A,b)
   ...
   αJ = α[Jlb]; 
   λJ = λ[Jlb];    
   ξstar = dot(b,β) + .25*dot(αJ,(αJ./λJ)) + .25*η*dot(γ[Jz],γ[Jz]);
   ...
   for j = 0:k_max
      ...
      aux[Iz] = zeros_Iz;
      zγ = aux;   
      aux_g_α = zeros(n); aux_g_α[Jlb] = αJ./λJ;
      g_α = .5*aux_g_α + .5*η*(V')*zγ;
      g_β = b + .5*η*A*zγ;
      ng_α = norm(g_α);
      ng_β = norm(g_β);
      ...
      Jz = indices[1:s];
      ξstar_old = ξstar;
      ξ = dot(b,β) + .25*dot(αJ,(αJ./λJ)) + .25*η*dot(γ[Jz],γ[Jz]);
